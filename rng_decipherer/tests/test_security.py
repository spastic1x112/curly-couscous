...
def test_mt19937_file_leak():
    ...
    try:
        ...
        env = os.environ.copy()
        env["PYTHONPATH"] = (
            f"{os.getcwd()}/rng_decipherer/src:{env.get('PYTHONPATH', '')}"
        )
        ...
        result = subprocess.run(
            [
                "python3",
                "rng_decipherer/src/rng_decipherer/cli.py",
                "mt19937",
                "--file",
                file_path,
            ],
        ...
        assert (
            "Error: Invalid integer value found in file." in result.stdout
            or "Error: Invalid integer value found in file." in result.stderr
        )
